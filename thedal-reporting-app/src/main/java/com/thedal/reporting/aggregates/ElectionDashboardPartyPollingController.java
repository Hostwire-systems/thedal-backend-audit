...
/**
 * Placeholder controller for party polling reporting
 */
@RestController
@RequestMapping("/api/aggregates/election/party-polling")
@RequiredArgsConstructor
@Slf4j
public class ElectionDashboardPartyPollingController {

    @GetMapping("/{accountId}/{electionId}")
    public ResponseEntity<?> get(@PathVariable Long accountId, @PathVariable Long electionId) {
        log.info("Party polling requested for account={} election={}", accountId, electionId);
        ...
        Map<String, Object> response = Map.of(
            ...
            "freshnessSeconds", 0L,
            "message", "Party polling data will be available when database is fully connected"
        );
    ...
    public ResponseEntity<?> recompute(@PathVariable Long accountId, @PathVariable Long electionId) {
        log.info("Party polling recompute requested for account={} election={}", accountId, electionId);
